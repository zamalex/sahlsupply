...
class ManageKeyScreen extends StatefulWidget {
  const ManageKeyScreen({Key? key}) : super(key: key);
...
class _ManageKeyScreenState extends State<ManageKeyScreen> {
  ...
  void _onSave() {
    if (_textEditingController.text.isEmpty) {
      context.showMessage(S.of(context).pleaseInputKey);
  ...
  void _onRemove() {
    context
        .showCupertinoConfirmDialog(content: S.of(context).confirmRemoveKey)
        .then((value) {
      if (value == true) {
  ...
  void _listenStateChanged(BuildContext _, ManageKeyState state) {
    state.whenOrNull(
      ...
      removeKeySuccess: () {
        context.showNoticeDialog(S.of(context).removeKeySuccess).then((value) {
          Navigator.pop(context);
        });
      },
      removeKeyFailure: () {
        context.showMessage(S.of(context).removeKeyFailed);
      },
      saveKeySuccess: () {
        context.showNoticeDialog(S.of(context).saveKeySuccess).then((value) {
          Navigator.pop(context);
        });
      },
      saveKeyFailure: () {
        context.showNoticeDialog(S.of(context).saveKeyFailed);
      },
      invalidKey: () {
        context.showNoticeDialog(S.of(context).invalidKey);
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(S.of(context).manage),
      ),
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(S.of(context).introAboutKey),
                    const SizedBox(height: 24),
                    TextField(
                      controller: _textEditingController,
                      decoration: InputDecoration(
                        border: const OutlineInputBorder(),
                        label: Text(S.of(context).putKeyHere),
                      ),
                    ),
                    const SizedBox(height: 12),
                    ElevatedButton.icon(
